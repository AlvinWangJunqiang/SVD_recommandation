...
def offline_similarity(dataMat,simMeas=cosSim,percentage):
    n = np.shape(dataMat)[1]
    offline_similarity = []
...
#            print a[i][j]
            offline_similarity[i][j] = simMeas(xformedItems[i, :].T, xformedItems[j, :].T) 
...
def svdEst_offline(dataMat, user, simMeas, item, percentage):
    n = np.shape(dataMat)[1]
    simTotal = 0.0;
    ratSimTotal = 0.0
    u, sigma, vt = la.svd(dataMat)
...
def recommend(dataMat, user, N=3, simMeas=cosSim, estMethod=svdEst_offline, percentage=0.9):
...
if __name__ == '__main__':
    pass
    os.chdir('C:/Users/GZS10228/plane_tickets/recomandation/')
    train_data = loadMovieLensTrain()
    test_data = loadMovieLensTest()
    time_1 = datetime.datetime.now()
    itemScores = recommend(dataMat, user, N=10, simMeas=cosSim, estMethod=svdEst_offline, percentage=0.9)
